...
export default class HintRecipeService{
    ...
    async getHintIngredient(ingredientIds: string[],userId?:number ,page: number=1, pageSize: number=10) {
        ...
        .select(['recipe.name as name','recipe.id as id','recipe.imageUrl as imageUrl',
        ])
        ...
        .andWhere("recipeIngredient.ingredientId IN (:...ingredientIds)", { ingredientIds })
        .groupBy("recipe.id")
        .having('priorityLevel/COUNT(recipeIngredient.id) >= :min', { min: 0.5 })
        .orderBy("priorityLevel", "DESC")
        if(userId){
            const user = await (await this.userRepository.getBy(userId)).leftJoinAndSelect('user.region','region').getOne()
            query.addSelect([
        ...
        const recipes = await query.limit(pageSize)
    ...
    async getHintUser(userId:number,page:number,pageSize:number){
        const user = await (await this.userRepository.getBy(userId)).leftJoinAndSelect('user.region','region').getOne()
        const recipes = await (await this.repository
            .createQueryBuilder())
            .select(['recipe.name as name','recipe.id as id','recipe.imageUrl as imageUrl'])
            .addSelect([
    ...
    async getHintSimilar(recipeId:number,page:number,pageSize:number){
        ...
        .getOne()
        const ingredientIds = recipe.recipeIngredients.map((recipeIngredient)=>recipeIngredient.ingredient.id)
        const recipes = await (await this.repository
        .createQueryBuilder())
        .innerJoin("recipe.recipeIngredients", "recipeIngredient")
        .select(['recipe.name as name','recipe.id as id','recipe.imageUrl as imageUrl'])
    ...
    async getHighlyAppreciated(page: number, pageSize: number) {

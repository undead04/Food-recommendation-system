...
export default class RecipeService extends BaseService<Recipe> {
  ...
  constructor() {
    super(Recipe, "recipe");
    this.reciepeIngredient = new BaseRepository(
      RecipeIngredient,
      "recipeIngredient"
    );
    this.recipeCategoryRepository = new BaseRepository(
      RecipeCategory,
      "recipeCategory"
    );
  }
  ...
  protected async validate(id: number, data: RecipeModel): Promise<void> {
    ...
    await Promise.all(
      data.categoryId.map(
        async (item) => await this.categoryService.getById(item)
      )
    );
    await Promise.all(
      data.recipeIngredient.map(
        async (item) => await this.ingredientService.getById(item.ingredientId)
      )
    );
    await this.regionService.getById(data.regionId);
    if (record && record.id !== id) {
      throw new CustomError(
        `Tên món ăn này đã tồn tại name = ${record.name}`,
        400,
        "name"
      );
    }
  ...
  async create(data: RecipeModel) {
    ...
    await dataSource.manager.transaction(async (transtionEntityManager) => {
      const recipeData = await this.repository.create(
      ...
      await this.reciepeIngredient.createArray(
        data.recipeIngredient.map((item) => ({
          recipe: { id: recipeData.id },
  ...
  async getFilter(
    ...
    if (ingredientId) {
      queryBuilder.andWhere(
        "recipeIngredient.ingredientId IN (:...ingredientId)",
        { ingredientId }
      );
    }
    if (categoryId) {
      queryBuilder.andWhere("recipaCategory.ingredientId IN (:...categoryId)", {
        categoryId,
      });
    }
    if (orderBy) {
  ...
  async createArray(models: RecipeModel[]) {
    ...
    await Promise.all(
      models.map(async (model) => await this.validate(0, model))
    );
  ...
  async update(id: number, data: RecipeModel) {
    ...
    await dataSource.transaction(async (transtionEntityManager) => {
      ...
      await this.reciepeIngredient.removeArray(
        record.recipeIngredients.map((item) => item.id),
        transtionEntityManager
      );
      await this.recipeCategoryRepository.removeArray(
        record.recipeCategorys.map((item) => item.id),
        transtionEntityManager
      );
      await this.recipeCategoryRepository.createArray(
        data.categoryId.map((item) => ({
